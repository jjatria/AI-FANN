...
sub fann_create_standard_array(uint32 $num_layers, CArray[int32] $layers is rw )
    returns fann is export is native(LIB) {*}
...
sub fann_run(fann, CArray[float] is rw) returns CArray[fann_type] is export is native(LIB) {*}
...
sub fann_get_layer_array(fann, CArray[uint32] is rw) is export is native(LIB) {*}
